...
docs_claim() {
  ...
  tr -s '[:space:]' ' ' <"$f" \
    | grep -c -F "\`scripts/test-gh-routing.sh\` ($EXPECTED_ROWS checks"
}

printf '\nthe count this suite is documented as running\n'
check "docs/GH_ACCOUNT_ROUTING.md says $EXPECTED_ROWS checks" \
      "$(docs_claim docs/GH_ACCOUNT_ROUTING.md)" 1
