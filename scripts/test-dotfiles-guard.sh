...
docs_claim() {
  ...
  tr -s '[:space:]' ' ' <"$f" \
    | grep -c -F "\`scripts/test-dotfiles-guard.sh\` ($EXPECTED_ROWS checks"
}

printf '\nthe count this suite is documented as running\n'
check "docs/DOTFILES_DEPLOY.md says $EXPECTED_ROWS checks" \
      "$(docs_claim docs/DOTFILES_DEPLOY.md)" 1
