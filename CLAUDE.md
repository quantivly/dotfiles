...
| `zsh/functions/system.sh` | Performance + Utilities + GNOME + Backup (22 functions) | `startup_monitor`, `system_health`, `has_command`, `confirm`, `gnome-status`, `backup-now`, `backup-status`, `backup-restore` |
...
- **Source of truth:** `resticprofile/profiles.toml` (policy: sources, excludes, retention, checks, schedule). **Copied** root-owned to `/etc/resticprofile/` by `backup-setup` — never symlinked (root runs its hooks; a user-writable config would be privilege escalation).
- **Machine-specific layer:** `~/.backup.local` (repo URLs, B2 keys, healthcheck URLs) from `examples/backup.local.template`. Created by `./install` on GNOME / `backup-init`. Plain `KEY=value` (consumed by both shell `source` and systemd `EnvironmentFile`).
...
- **Restore correctness (LVM-on-LUKS):** regenerate — do not restore — `/etc/fstab`, `/etc/crypttab`, `/etc/machine-id`, `ssh_host_*`. Timeshift is file-level rollback only (known LVM-on-LUKS bare-metal restore bug).

Commands: `backup-init`, `backup-setup`, `backup-now`, `backup-status`, `backup-snapshots`, `backup-check`, `backup-restore`, `backup-mount`, `backup-prune`, `backup-luks-header`, `backup-kit`.
...
- **Backups:** `backup-status` (health), `systemctl list-timers | grep restic`, `resticprofile -c /etc/resticprofile/profiles.toml show` (validate config)
