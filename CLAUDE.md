...
- **Two traps specific to this area.** `findmnt --verify` errors on "unreachable on boot" for a `nofail` entry whenever the disk is undocked *or* its mount point directory is absent, so it can only be used to compare **parse-error counts**, never as a pass/fail gate — gating on its exit code rejects every correct entry in exactly the states that need fixing. And **`test -e "$repo/config"` is not a docked test**: a leftover directory on the root filesystem satisfies it while the disk is unmounted, so `backup-status`/`backup-doctor` use `findmnt --mountpoint`. `scripts/test-backup-external.sh` pins both (root-free; run it after touching the external steps).
