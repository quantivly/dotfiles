...
classifying (DO-751), answered from ``sources/linear.json``/``sources/github.json`` on disk with
no network — see ``reconcile.lookup_tracked``.
...
def _text_line(r: dict) -> str:
    ...
    if r["status"] == "not_found":
        return f"{r['key']}: not found — the snapshot is trustworthy and does not name this subject"
    if r["status"] == "unknown":
        return f"{r['key']}: unknown — {r['reason']}"
    return f"{r['key']}: not applicable — not a Linear issue or owner/repo#n PR key"


def run_tracked(ctx: Context, keys: list[str]) -> dict:
    return lookup_tracked(ctx, keys)
