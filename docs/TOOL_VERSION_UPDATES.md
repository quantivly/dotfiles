...
- **Symlinked** (not copied) to `~/.config/mise/config.toml` by `dotfiles/install` — see the maintainer's record at the end of this page
...
## Maintainer's record: the symlink, and the failure mode it has

The record behind the **mise** rules in [CLAUDE.md](../CLAUDE.md). It corrects the
"Source of Truth Architecture" section above on one point of fact — `~/.config/mise/config.toml`
is a **symlink**, not a copy — and that correction is the whole reason the drift below went
unnoticed.
